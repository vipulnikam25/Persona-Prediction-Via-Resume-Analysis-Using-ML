...
class MainWindow:
    ...
    def __init__(self, master):
        ...
        self.master.title("Online Test")
        self.master.config(bg="#009FBF")

        f = Frame(self.master, height=1080, width=1920,
        ...
        self.mainTitle = Label(f, text="Online Test", fg="red", bg="azure", font=("Helvetica", 30, "bold italic")).place(
            x=400, y=200)
...
class Register:
    ...
    def __init__(self, master):
        ...
        global f1
        f1 = Frame(self.master, height=1080, width=1920,
                   bg="azure", relief="ridge", bd=20)
        f1.propagate(0)
        f1.pack()

        # self.regNow = PhotoImage(
        #     file="E:\OSTL\MiniProject\Resources\\RegisterNow.png")
        ...
        self.name = Label(f1, text="First Name : ",
                          bg="azure", font=("Times New Roman", 10))
        self.lname = Label(f1, text="Last name : ",
                           bg="azure", font=("Times New Roman", 10))
        self.email = Label(f1, text="Email id : ", bg="azure",
                           font=("Times New Roman", 10))
        self.uname = Label(f1, text="Username :", bg="azure",
                           font=("Times New Roman", 10))
        self.pw = Label(f1, text="Enter password : ",
                        bg="azure", font=("Times New Roman", 10))
    ...
    def check(self, l1):
        ...
        for i in range(5):
            ...
            if len(l1[i]) == 0:
                self.l = Label(
                    f1, text="! You cannot leave this empty", fg='red', bg="azure")
                self.l.place(x=400, y=ht)
            else:
                self.l = Label(
                    f1, text="! You cannot leave this empty", bg="azure", fg="azure")
                self.l.place(x=400, y=ht)
                f = f + 1
        if l1[2].find("@") == -1 and l1[2].find(".") == -1 and len(l1[2]) != 0:
            self.l = Label(
                f1, text="! Please enter a valid email id", bg="azure", fg="red")
            self.l.place(x=400, y=200)
            s = 1
        else:
            if (len(l1[2]) > 0):
                self.l = Label(
                    f1, text="! Please enter a valid email id", bg="azure", fg="azure")
                self.l.place(x=400, y=200)
        if len(l1[4]) < 8 and len(l1[4]) != 0:
            self.l = Label(
                f1, text="! Password must atleast have 8 characters", bg="azure", fg="red")
            self.l.place(x=400, y=300)
        else:
            if (len(l1[4]) > 0):
                self.l = Label(
                    f1, text="! Password must atleast have 8 characters", bg="azure", fg="azure")
                self.l.place(x=400, y=300)
    ...
    def c_submit(self):
        conn = pymysql.connect(
            host='localhost', database='personality', user='root', password='root')
        cursor = conn.cursor()
        name = self.tname.get()
...
class Login:
    ...
    def __init__(self, master):
        ...
        global f2
        f2 = Frame(self.master, height=1080, width=1920,
                   bg="azure", relief="ridge", bd=20)
        f2.propagate(0)
        f2.pack()

        self.l1 = Label(f2, text="Enter Username: ",
                        bg="azure", font=("Times New Roman", 20))
        self.e1 = Entry(f2, width=30)
        self.l2 = Label(f2, text="Enter Password: ",
                        bg="azure", font=("Times New Roman", 20))
...
class Account:
    ...
    def __init__(self, master, u):
        ...
        self.master.config(bg="#009FBF")
        f3 = Frame(mAcc, height=1080, width=1920,
                   bg="azure", relief="ridge", bd=20)
        f3.propagate(0)
        f3.pack()
        conn = pymysql.connect(
        ...
        choices = ['Technical', 'Logical',
                   'Personality', 'English/Grammer', 'Aptitude']
        global variable
        variable = StringVar(root)
        variable.set('Select Subject')
        self.subject = OptionMenu(f3, variable, *choices).place(x=500, y=400)
        ...
        self.takeQuiz.place(x=600, y=500)
...
class Quiz:
    def __init__(self, master, u):
        ...
        print('Subject')
        print(sub)
        query = "select q_id from questions where subject='"+sub+"'"
        print(query)
        ...
        while len(s1) > 0:
            s = "select question_description from questions where q_id=%d"
            id = s1.pop()
            ...
            s = "select opt_a,opt_b,opt_c,opt_d from questions where q_id=%d"
            arg = (id)
            cursor.execute(s % arg)
            options.append(list(cursor.fetchone()))

            s = "select ans from questions where q_id=%d"
            arg = (id)
            cursor.execute(s % arg)
    ...
    def create_q(self, master, qno):
        qLabel = Label(master, text=questions[qno], bg='azure', font=(
            "Times New Roman", 20))
    ...
    def Submit(self):
        ...
        # cursor.execute(s % arg)
        q1 = "insert into testscore(username,subj,score) values('%s','%s',%d)"
        arg1 = (userName, variable.get(), s)
        cursor.execute(q1 % arg1)
        conn.commit()
        cursor.close()
        conn.close()
